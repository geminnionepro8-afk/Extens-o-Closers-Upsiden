...
async function initPainel() {
  ...
  try {
    ...
    const verifiedIcon = userData.isAdmin ? `<svg class="role-badge" viewBox="0 0 24 24"><path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/></svg>` : '';
    document.getElementById('user-display-role').innerHTML = (userData.isAdmin ? 'Administrador' : 'Closer') + verifiedIcon;
    ...
    if (umpName) umpName.innerHTML = (userData.nome || 'Usuario') + nameVerifiedIcon;
...
function _runDelegatedEvent(e, attrName) {
  ...
  if (match) {
    ...
    if (match[2]) {
      args = match[2].split(',').map(s => {
         let v = s.trim();
         if (v === 'this.value') return el.value;
         if (v === 'this.checked') return el.checked;
         return v.replace(/^['"]|['"]$/g, '');
      });
    }
...
document.addEventListener('change', e => _runDelegatedEvent(e, 'data-change'));
document.addEventListener('input', e => _runDelegatedEvent(e, 'data-input'));
